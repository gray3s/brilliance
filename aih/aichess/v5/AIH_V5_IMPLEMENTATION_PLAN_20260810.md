...
This file supplements `AIH_V5_PROJECT_GOALS_20260810.md`. The goals file defines the target behavior. This implementation plan describes one practical way to build it.
...
- Repeat-run HTML reporter: aggregates `registration_status_run_*.csv` files.
...
aih/aichess/v5/
  ...
  runs/
    aih_v5_pairwise_prototype_YYYYMMDD/
    aih_v5_random_repeat_YYYYMMDD_HHMMSS/
  data/
    AIH_V5_CURRENT_RUN_AGGREGATE.jsonl
  AIH_V5_REGISTRATION_STATUS.csv
  AIH_V5_REGISTRATION_DIAGNOSTICS.log
  AIH_V5_PRELIMINARY_RESULTS_YYYYMMDD.html
  AIH_V5_RANDOM_REPEAT_AGGREGATE_LATEST.html
```

The exact language and directory names can vary, but preserve the behavior and data contracts.
...
5. Save every candidate registration result to CSV.
...
10. Save run JSONL/summary artifacts.
11. Generate HTML output.
12. Open the HTML output by default.
...
The registration CSV must include:

```text
candidate,status,reason,elapsed_seconds,timeout_seconds,avg_move_seconds,move_attempts
```

At registration time, `avg_move_seconds` and `move_attempts` may be blank/zero. After the tournament completes, backfill real per-agent move timing for that same run before saving the per-run CSV snapshot.
...
- Preserve per-run registration CSV snapshots.
...
```text
registration_status_run_*.csv
```

Supported flags:

```bash
./bin/aih_v5_repeat_html --nruns=N <repeat-folder>
./bin/aih_v5_repeat_html --nruns=0 <repeat-folder>
```

Behavior:

- `--nruns=0` reads all saved run CSVs.
- `--nruns=N` reads the oldest N CSVs in the folder for the current target.
...
- Repeat HTML reads only CSV files.
...
- `AIH_V5_PROJECT_GOALS_20260810.md`
- `AIH_V5_IMPLEMENTATION_PLAN_20260810.md`
- `AIH_V5_20260810_demo.html`

The demo HTML may initially be a placeholder. Replace it with a real multi-run summary after a validated longer run.
...
- Open the final aggregate HTML by default.
...
- Per-run CSV files appear in a timestamped repeat folder.
