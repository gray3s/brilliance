...
The repeat HTML aggregator should read only these CSV files for registration aggregate display. It should handle different candidate sets across runs and compute per-agent totals from whatever rows exist.
...
- Read `registration_status_run_*.csv` files from the repeat-run folder.
- Support `--nruns=N`.
- Support `--nruns=0` to aggregate all saved CSVs in that folder.
- For the current target, when `--nruns=N`, summarize the oldest N CSV files in the folder.
...
This file and the linked demo HTML report are the public target artifacts for the AIH v5 prompt/build challenge. The implementation can be changed, but the target behavior is:

- local-first operation;
- registration filtering;
- bounded tournament execution;
- per-run CSV persistence;
...
```text
Create AIH v5 as a local-first command-line benchmark named ./bin/aih_v5. Implement registration filtering for local Ollama agents, run a compact chess-harness tournament over registered agents, save per-run registration CSV snapshots, support repeated runs with --nruns and --minregs, and generate browser-opened single-run and aggregate HTML summaries. Follow the runtime architecture, default values, CSV contract, terminal output contract, and HTML reporting contract in AIH_V5_PROJECT_GOALS_20260810.md.
```
